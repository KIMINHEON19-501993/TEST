    	 ...
    	 $.ajax(
    			 {
    				 ...
    				 data:{deptno : Vdeptno},
    				 dataType:'text',
    				 success:function(data){  // data 는 변수 이름
...
<table>
	...
	<c:forEach var="empDept" items="${listEmp}">
		<tr><td>${empDept.empno }</td><td>${empDept.ename }</td>
			<td>${empDept.job }</td>
			<td>${empDept.deptno} 
			    <input type="button" id="btn_idCheck" value="부서명" onmouseover="getDeptName(${empDept.deptno })" onclick="getDept1(${empDept.deptno})">
			</td>
			<td>${empDept.loc }</td>
		</tr>
	</c:forEach>
	...
	deptName:  <input type="text" id="getDept1"  readonly="readonly"><p>
